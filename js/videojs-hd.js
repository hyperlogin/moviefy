        ...
        function HDtoggle () {
         ...
         var HDsrc = $("#moviestream").find( "video" ).attr("HD"); 
		 /* Value of HD attribute in video tag is saved in HDsrc. */
         var noHDsrc = $("#moviestream").find( "video" ).attr("nonHD");
         ...
         videojs("moviestream").src([{type: "video/mp4", src: noHDsrc }]);
		 
         videojs("moviestream").play();
		...
		document.body.appendChild(css);
         videojs("moviestream").src([{type: "video/mp4", src: HDsrc }]);
         videojs("moviestream").play();
          /*This automatically plays the video when we  click on HD button to change the source.*/
		 HD1 = true;
         }
